...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Carbon Valuation After Treatment", tabName = "tab_6")
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "tab_3",
              ...
              fluidRow(
                box(leafletOutput("my_graph7", height = 432)),
                box(title = "Departure from Fire Regime",
      ...
      sidebarLayout(
        sidebarPanel(
          selectInput("Climate_Model", "Climate Model:", choices = unique(carbon$Climate_Model)), 
...
server <- function(input, output){
  ...
  output$distPlot <- renderPlot({
    ...
    ggplot(subset(carbon, Year == Years & Climate_Model == pick_model), aes(x = Discount_Rate, y = DifferenceToDate)) +
      ...
      theme_classic() +
      ylab("Cumulative $/metric tonne CO2e") +
      xlab("Applied Discount Rate") + 
      ggtitle("Value of Carbon Sequestered From Treatment (in $US)") +
      theme(plot.title = element_text(hjust = 0.5)) +
      scale_y_continuous(expand = c(0,0), limits = (c(-300000, 13600000))) +
      coord_flip() + 
      guides(fill = FALSE)
  })
